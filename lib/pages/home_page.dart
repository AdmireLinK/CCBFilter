...
class _HomePageState extends State<HomePage>
  ...
  bool _useAnimeOnly = false; // false=All.json, true=Anime.json
        ...
        Row(
          children: [
            ...
            SizedBox(
              ...
              child: Checkbox(
                value: fuzzy,
                onChanged: onFuzzyChanged,
                visualDensity: VisualDensity.compact,
              ),
            ),
            const Text('模糊', style: TextStyle(fontSize: 10)),
          ],
        ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(
            '共 ${_filteredCharacters.length} 条',
              ...
              children: [
                ...
                if (char.metaTags.isNotEmpty) ...[
                  ...
                  Wrap(
                    spacing: 4,
                    runSpacing: 4,
                    children: char.metaTags.take(8).map((tag) {
                      final matches = _matchesTags([tag]);
                      return Container(
  ...
  bool _matchesTags(List<String> charTags) {
    if (_selectedTags.isEmpty) return false;

    final charTagsLower = charTags.map((t) => t.toLowerCase()).toSet();
    for (final tag in _selectedTags) {
      if (charTagsLower.contains(tag.toLowerCase())) {
        return true;
      }
    }
